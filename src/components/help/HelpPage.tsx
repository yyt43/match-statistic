...
const content: Record<AppLanguage, HelpCopy> = {
  zh: {
    ...
    features: '功能概览',
    featureItems: [
      { icon: <Users className="w-5 h-5" />, title: '多小组管理', description: '支持 1-20 个小组独立配置和开赛；编号范围为 A01-Z99，缺失时可一键补齐且不覆盖已有编号。' },
      ...
      { icon: <FileUp className="w-5 h-5" />, title: '赛果导入', description: '导入腾讯文档 XLSX，按选手编号匹配比赛，逐场核验截图后写入结果。' },
      ...
      { icon: <Download className="w-5 h-5" />, title: '导出工具', description: '排行榜、对阵表和总表可导出为 PNG 或 Excel，并包含选手编号与 UID；赛事数据可导出为 JSON。' },
      ...
      { icon: <Keyboard className="w-5 h-5" />, title: '快速录分', description: '支持多小组同屏、搜索、筛选、具体比分键盘录入、已录比分显示和颜色反馈后跳转。' },
      { icon: <Shield className="w-5 h-5" />, title: '存储体检', description: '检查主备数据、双存储一致性、快照和审计记录，并可安全重写修复。' },
    ...
    faq: '常见问题',
    faqItems: [
      { question: '如何修改已录入的比赛结果？', answer: '可直接修改当前比赛结果，也可以使用 Ctrl/Cmd+Z 撤销上一操作；撤回按钮已有赛果时保留本轮对阵并清空结果，未录分时则撤回至上一轮或开赛前设置。' },
  ...
  en: {
    ...
    features: 'Features',
    featureItems: [
      { icon: <Users className="w-5 h-5" />, title: 'Multi-group management', description: 'Configure and run 1-20 groups. Codes use A01-Z99, and missing codes can be filled without overwriting existing ones.' },
      ...
      { icon: <Download className="w-5 h-5" />, title: 'Export tools', description: 'Export rankings, match tables, and summaries as PNG or Excel with participant codes and UIDs, or save tournament data as JSON.' },
      ...
      { icon: <Keyboard className="w-5 h-5" />, title: 'Quick score entry', description: 'View multiple groups together, search, filter, enter detailed keyboard scores, view recorded scores, and advance after clear feedback.' },
      { icon: <Shield className="w-5 h-5" />, title: 'Storage health', description: 'Inspect primary and backup records, mirrors, snapshots, and audit data with safe rewrite repair.' },
    ...
    faq: 'FAQ',
    faqItems: [
      { question: 'How do I correct an entered result?', answer: 'Edit the current match directly, use Ctrl/Cmd+Z for the last operation, or use the context-aware undo action: it keeps current pairings when results exist, or returns an empty later round to the previous round or setup.' },
