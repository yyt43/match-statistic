...
- 统一赛制草稿：同一套设置可应用至本小组或全部小组，开赛前确认赛事信息
- 独立开赛设置页：未开赛时隐藏排行榜和对阵表，小组操作集中在当前小组，主区域按赛制、选手和赛事数据三列布局
- 瑞士轮 / 单败淘汰双赛制：支持常见比赛流程与淘汰结构
- Excel 一键导入：支持文本、CSV、TXT、XLSX 批量导入选手名单
- 多表格工作簿导入：按 sheet 名自动识别小组，批量分组录入
- 统一选手档案：昵称、选手编号、9 位 UID、QQ 使用同一导入模板和同一编辑列表
- 选手编辑保护：手动修改昵称后切换小组再返回，原小组内容不会丢失
- 当前小组切换：在“当前小组”区域直接点击 A组、B组、C组、D组
...
- 导出一致性：排行榜、图片排行榜、对阵表和总表均包含选手编号与 UID
- 移动端适配：赛前工作区自动切换为纵向布局，顶部统计和操作栏支持换行显示
...
3. 如果工作簿包含多个 sheet，会跳过 `报名信息`、`汇总`、`总表` 等总览页，再按 sheet 名称匹配小组。
...
- `A组`、`B组`、`C组` 会分别映射为三组小组
- `报名信息`、`汇总`、`总表`、`All` 等总览 Sheet 会自动跳过，不会创建为额外小组
- 当总览 Sheet 与分组 Sheet 包含相同 UID 时，以 `A组`、`B组` 等分组名单为准
- 每个 sheet 内的有效姓名会自动收集并去重
- 若某个 sheet 为空或未识别到姓名，则自动跳过，不会创建空小组
- 若只有一个 sheet，则按传统单组名单导入行为处理

这种方式很适合赛前把不同组别名单分表保存、统一上传、减少重复录入。若分组表没有选手编号，导入后可点击“一键编号”按小组补齐。

## 选手档案与 UID

选手管理使用统一的选手信息表，不再需要单独选择“通用模板”或“诗意杯模板”。昵称、选手编号、UID、QQ 都按同一流程导入和编辑。

小组切换已整合到“当前小组”区域，点击 A组、B组、C组、D组即可切换。

选手信息表导入入口位于开赛设置页右侧“赛事数据”列；选手列表严格按选手编号排序。编号格式为 `A01-Z99`，每组最多 99 个编号。表格缺少编号时，可使用“一键编号”只补齐空编号，已有编号不会被覆盖。

选手列表中的昵称、编号、UID、QQ 可直接编辑。提交编辑后会写回选手原所属小组，切换小组查看其他名单时不会丢失已输入内容。
...
选手编号优先从导入表格读取；仅在存在空编号时可使用“一键编号”按小组补齐。UID 和 QQ 可以为空；只要填写，就会执行格式和唯一性校验。

选手档案锁定后，UID、选手编号和内部 Player.id 的映射不可修改。结果导入时会同时检查选手编号和 UID，二者不一致时进入裁判异常队列。

系统不再为每场比赛设置“确认/异议”按钮。比分有误时，裁判直接修改结果；导入结果需要改判时填写原因，系统保留改判记录并重新计算后续未开赛轮次。
...
导入赛果时不需要选择阶段、小组或轮次。系统会按选手编号跨全部小组自动定位选手，并默认匹配其小组当前轮次；因此一张表可以同时录入多个小组的比赛结果。
...
- `1` / `2`：判定左侧或右侧选手直落获胜，例如 BO3 的 `2-0`。
- `3+`：按详细赛果按钮上显示的数字录入具体比分；BO3 中 `3` 为左胜 `2-1`、`4` 为右胜 `2-1`。
- `D`：记录双负。
- `N`：跳到下一场待录比赛。
- `←` / `→`：切换上一场或下一场比赛。

快速录分列表和当前比赛信息区都会显示已录具体比分，例如“左胜 2-1”或“已录 1-2”。
