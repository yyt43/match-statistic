              ...
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-300">
                    ...
                    <span className="block mt-0.5 text-slate-400">
                      赛前弃赛场次<strong className="text-amber-300">整体从该网络剔除</strong>：既不把对方加入对手集合，也在"有效战绩"中扣除该场胜/负；同时赛前弃赛败方（弃赛者本人）个人不记 losses——例：1-1 选手第 3 轮赛前弃赛，个人胜率仍为 1/(1+1)=0.5，计入对手胜率时仍按 1 胜 / 2 场计算。
                    </span>
                  </p>
                </div>
                <div>
              ...
              <ul className="text-sm text-slate-300 space-y-1 list-disc list-inside">
                ...
                <li>
                  ...
                  <span className="block pl-4 text-slate-400">
                    ...
                    <br />· 弃赛者视角例：某选手已 1-1，第 3 轮赛前弃赛负 → 个人<strong>不记败场</strong>，个人胜率仍为 1/(1+1)=0.5；计入对手胜率时仍按 1 胜 / 2 场计算。
                  </span>
                </li>
                <li>
        ...
        <section className="mb-10">
          ...
          <div className="space-y-4">
            ...
            <FAQItem
              ...
              answer="分两种场景：① 赛前弃赛（单场）：对阵卡片展开后选择「右弃·左胜」或「左弃·右胜」。胜方记个人胜场，弃赛方不记个人败场，该场整体不计入对手胜率网络。② 赛后弃赛（全局/手动弃赛）：在右侧控制面板的「弃赛管理」区域，点击选手旁的「弃赛」按钮标记弃权。标记后该选手后续轮次不再参与配对；如需恢复，可在「已弃赛」列表中点击「恢复」按钮。单败淘汰中比赛败者会被自动标记为「淘汰」，与手动弃赛是两种不同状态：淘汰选手在排行榜显示灰色「淘汰」标签，仍保留在排行榜中；弃赛选手显示红色「弃赛」标签并加删除线，排在最后。两者（淘汰/赛后弃赛）均不参与后续轮次配对，但赛前弃赛为单场级处理，不影响选手后续其它轮次的配对。"
            />
            <FAQItem
            ...
            <FAQItem
              ...
              answer="瑞士轮中，当赛程进入后半段（R4、R5、R6），选手交手历史增多，可能出现「不重复对阵」与「战绩严格分层」两个硬约束冲突的情况——某些选手找不到合法对手。此时按规则7，下移组剩余者直接轮空（最终残余的下移组会先优先内部互相匹配，仍无法匹配的才轮空）。轮空人数始终为偶数（0、2、4 个），且轮空场次按赛制自动填充比分（BO1=1-0, BO3=2-0...），计入局胜率与对手胜率（轮空对手视为 0 胜 / 1 场的虚拟对手，赛后弃赛对手一样按聚合公式胜场和/总场次和计入）；赛前弃赛场次则<strong>不计入</strong>这一网络，两者泾渭分明。这是规则设计的正常行为，不是 bug。"
            />
            <FAQItem
