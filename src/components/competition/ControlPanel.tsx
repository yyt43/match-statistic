...
import { PlayerManagerHeader, PlayerManagerList } from '../players/PlayerManager';
...
export function ControlPanel({ onShowConfirm, onShowConfirmAll }: ControlPanelProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-800/40 border border-slate-700/40 rounded-2xl overflow-hidden">
      ...
      {/* 小组管理 */}
      <div className="border-b border-slate-700/50 bg-slate-800/40 px-4 py-2">
        <button
        ...
        {showGroupManager && (
            <div className="mt-3 max-h-40 space-y-2 overflow-y-auto overscroll-contain pr-1">
            {hasAnyStarted && (
      ...
      {isSetup && (
        <div className="border-b border-slate-700/50 bg-slate-800/40 px-4 py-2">
          <PlayerManagerHeader
            expanded={showPlayerManager}
            onToggle={() => setShowPlayerManager(current => !current)}
          />
        </div>
      )}
      ...
      <div ref={controlScrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {isSetup && showPlayerManager && <PlayerManagerList />}
