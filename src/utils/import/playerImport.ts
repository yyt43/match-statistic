...
export async function getWorkbookImportColumnChoicesFromFile(
  file: File
): Promise<SheetColumnChoice[]> {
  return (await parseWorkbookFile(file)).map(sheet => {
    const { headers } = normalizeWorkbookRows(sheet.rows);
    return {
...
export async function parsePlayerNamesFromExcel(file: File, selectedColumns?: Record<string, string>): Promise<string[]> {
  const groups = await parsePlayerGroupsFromExcel(file, selectedColumns);
...
export async function parsePlayerGroupsFromExcel(file: File, selectedColumns?: Record<string, string>): Promise<ParsedGroupImport[]> {
  const groups: ParsedGroupImport[] = [];
  for (const sheet of await parseWorkbookFile(file)) {
    const { headers, dataRows } = normalizeWorkbookRows(sheet.rows);
    const selectedHeader = resolveSelectedNameColumn(sheet.name, headers, selectedColumns);
    const columnName = selectedHeader
      ? headers.find(header => normalizeColumnLabel(header) === normalizeColumnLabel(selectedHeader)) ?? headers[0]
      : headers[0];
    const values: string[] = [];
    for (const row of dataRows) {
      const value = columnName ? row[columnName] ?? row[headers[0] ?? ''] ?? '' : '';
    ...
    const summary = summarizePlayerNameInput(values.join('\n'));
    if (summary.validNames.length === 0) continue;
    groups.push({
      groupName: (sheet.name || '新小组').trim() || '新小组',
      names: summary.validNames,
    });
  }
  return groups;
...
async function parseWorkbookFile(file: File): Promise<ParsedWorkbookSheet[]> {
  ...
  try {
    return await new Promise<ParsedWorkbookSheet[]>((resolve, reject) => {
