...
本应用为纯前端单页应用，所有数据存储在浏览器本地（localStorage），无需后端服务。采用 React + TypeScript 构建，使用 Tailwind CSS 进行样式设计，Zustand 进行全局状态管理。随后扩展了多小组比赛管理、多表格 Excel 导入、静态站点部署等能力，并将业务逻辑拆分为纯函数与 store 层，保证可维护性和测试性。

```mermaid
flowchart TD
    A["用户界面层 (React Components)"] --> B["状态管理层 (Zustand Store)"]
    B --> C["业务逻辑层 (Utils)"]
    C --> D["数据持久层 (localStorage + 快照 + 备份)"]
    C --> E["导出能力 (html2canvas / xlsx / JSON)"]
    C --> F["Excel 导入解析 (xlsx + sheet 分组逻辑)"]
    B -.subscribe.- G["存储状态通知 (storageStatus 发布订阅)"]
    H["GitHub Pages / Vite base 配置"] --> A
```
...
- **状态管理**：Zustand 5（单一 store 管理赛事、小组状态、当前轮次视图、随机生成进度等）
- **路由**：React Router 7
- **数据持久化**：localStorage（主 key + backup key 双写，带版本化 envelope 和快照恢复）
- **图标库**：Lucide React
- **导出能力**：html2canvas（图片）、xlsx（Excel）、原生 Blob 下载（JSON）
- **Excel 导入**：`xlsx` 读取工作簿，支持文本、CSV/TXT、单表和多 sheet 工作簿；每个 sheet 名称作为小组名进行解析
- **站点部署**：Vite `base` 配置 + GitHub Pages 工作流，适配静态托管部署，提供中文元信息和搜索关键词
...
│   ├── playerImport.ts         # 文本和 Excel 导入解析，支持多 sheet 分组
...
interface Player {
  id: string;
  name: string;
  points: number;                  // 积分（胜+1 / 负0 / 双负0 / 轮空+1）
  wins: number;
  losses: number;
...
全局状态由 `src/store/useTournamentStore.ts` 单一 store 管理，主要职责：

- **赛事级别操作**：初始化、加载/导入、更新名称、小组增删/切换、批量配置、重置比赛和导入赛事快照
- **小组级别操作**：选手增删改、弃赛切换、轮次/赛制/局制设置、每轮赛制和当前小组切换
- **比赛流程**：开始赛事、生成下一轮（单个/全部小组）、撤回上一轮、录入结果、修改对阵、生成加赛
- **视图状态**：当前查看的轮次（viewRound）、随机生成进度、当前赛事状态
- **派生数据**：`getRankedPlayers()`、`getMatchesForRound()`、`isCurrentRoundComplete()`、`useCurrentGroup()`

### 5.2 Excel 导入解析设计

Excel 导入逻辑集中在 `src/utils/playerImport.ts`，职责如下：

- 处理文本名单输入：对换行、逗号、分号等分隔符统一清洗
- 处理工作簿导入：读取所有 sheet，并保留 `sheetName -> groupName` 的映射关系
- 对每个组的姓名数据去空白、去重复、过滤表头字段（如 `姓名`、`name`）
- 允许空 sheet 被忽略，避免创建无效空组
- 输出统一结构：`ParsedGroupImport[]`，包含 `groupName` 和 `names`

这样可以让 UI 层只关注“导入后的组列表”，不用重复处理底层 Excel 细节。
...
单败淘汰逻辑位于 `src/utils/swissPairing.ts` 和 `src/utils/playoffs.ts`，核心机制包括：

- 每轮根据胜者继续保留，败者淘汰
- 淘汰顺序用于最终排名排序
- 允许对阵编辑、随机分配和按录入顺序重排
- 若同分选手破分指标完全相同，则触发加赛逻辑并生成 playoff 匹配

这部分逻辑和瑞士轮逻辑保持分离，UI 层通过 `pairingType` 区分赛制，并在 store 中统一调度。
