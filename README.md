...
2. Excel 批量导入：上传 `.xlsx` / `.xls` / `.csv` / `.txt` 文件，系统会读取工作簿中的所有非空单元格，并自动清洗出可用姓名。
3. 多表格 Excel 导入：如果工作簿中含有多个 sheet，每个 sheet 名称会被当作一个小组名，系统会按表名自动拆分成多个小组并批量导入。适合一份 Excel 文件里同时保存多个组别名单。

录入过程会自动去除空白、重复项、表头字段（例如 `姓名` / `name`），避免名单中出现空值和重复选手。适合大规模赛前报名、比赛名单整理和跨表格迁移。
...
当常规比赛结束、所有破分指标仍相同时，每个同分组分别抽签。加赛允许与常规赛对手再次交手：
...
加赛不影响常规场次、局数或对手历史；最终加赛名次按比赛路径计算，不按累计加赛胜场推断。首阶段改判会清除该同分组依赖它的后续阶段；撤回常规轮次会清除失效加赛。旧版加赛需先导出备份，再使用「清除加赛」重新抽签。

下移组在无法完整配对时仍先完成最大的可行部分配对，然后让剩余选手继续下移。每次常规轮空在SOS及SOSOS中分别计入一次虚拟对手贡献，不再合并多次轮空。
